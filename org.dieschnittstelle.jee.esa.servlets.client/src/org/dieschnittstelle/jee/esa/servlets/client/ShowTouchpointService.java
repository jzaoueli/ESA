...
public class ShowTouchpointService {
	...
	public void run() {
		...
		if (touchpoints != null && touchpoints.size() > 0) {
			...
			System.err.println("TestTouchpointdeleted : "+ touchpoints.get(0));
		}
	...
	public void deleteTouchpoint(AbstractTouchpoint tp) {
		...
		int idtp = tp.getId();
		logger.info("delete Object()" + idtp);
		HttpDelete deletetp = new HttpDelete("http://localhost:8888/org.dieschnittstelle.jee.esa.servlets/api/touchpoints/" + idtp);
		HttpResponse response = client.execute(deletetp);
	...
	public AbstractTouchpoint createNewTouchpoint(AbstractTouchpoint tp) {
		...
		try {
			...
			System.err.println("received Object: " + receivedObj);
			// cleanup the request
			EntityUtils.consume(response.getEntity());
